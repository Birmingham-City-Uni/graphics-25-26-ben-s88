...
Eigen::Matrix4f projectionMatrix(int height, int width, float horzFov = 70.f*M_PI/180.f, float zFar = 10.f, float zNear = 0.1f)
{
	...
	projection <<
	1 / tanf(0.5 * horzFov), 0.f, 0.f, 0.f,
	0.f, 1 / tanf(0.5 * vertFov), 0.f, 0.f,
	...
	const std::vector<uint8_t>& albedoTexture, int texWidth, int texHeight)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			Eigen::Vector3f albedo{texColor.r, texColor.g, texColor.b};
	...
	int width, int height)
{
	for (int i = 0; i < mesh.vFaces.size(); ++i) {
		...
		t.screen[0] = Eigen::Vector3f(width * (vClip0.x() + 1) / 2, height * (vClip0.y() + 1) / 2, vClip0.z());
		t.screen[1] = Eigen::Vector3f(width * (vClip1.x() + 1) / 2, height * (vClip1.y() + 1) / 2, vClip1.z());
		t.screen[2] = Eigen::Vector3f(width * (vClip2.x() + 1) / 2, height * (vClip2.y() + 1) / 2, vClip2.z());
		// *** END YOUR CODE ***
