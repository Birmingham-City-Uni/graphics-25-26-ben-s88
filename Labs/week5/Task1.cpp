	...
	const Eigen::Vector3f& albedo)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			Eigen::Vector3f normP{b0 * t.norms[0] + b1 * t.norms[0] + b2 * t.norms[2]};
...
int main()
{
	...
	//lights.emplace_back(new DirectionalLight(Eigen::Vector3f(0.2f, 0.2f, 0.2f), Eigen::Vector3f(1.f, 0.f, 0.0f)));
	//lights.emplace_back(new PointLight(Eigen::Vector3f(0.5f, 0.5f, 0.5f), Eigen::Vector3f(0.5f, 0.f, 0.0f)));
	lights.emplace_back(new SpotLight(Eigen::Vector3f(10.0f, 0.0f, 0.0f), Eigen::Vector3f(1.f, 0.f, 0.0f), Eigen::Vector3f(-1, 0, 0), M_PI/4));
	// *** END YOUR CODE ***
